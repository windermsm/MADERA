 ...
 */public class GenerarDocumentoFEL {
    ...
    public  Resultado generarFacturaElectronica(String direccion, int pIdFactura, String pNombreVenta, String pEnlaceConexion, 
        ...
        try {
            ...
            cadena = generaFacturaXML(pIdFactura, pNombreVenta, pPantalla, pOpcion, pUsuario, pTerminal, pTipoDocumento);
            ...
            try {
                ...
                System.out.println("Cadena FEL: " + cadena);
    ...
    private String generaFacturaXML(int pIdFactura, String pNombreVenta, String pPantalla, String pOpcion, 
        ...
        for(int x=0; x<detalle.getProductos().size(); x++) {
            ...
            String xml_t =  "		<Productos>\n" +
                            ...
                            "			<ImpNeto>" + producto.getImpNeto() + "</ImpNeto>\n" +
            ...
            total_bruto = matematica.aproxima(total_bruto + producto.getImpBruto(), 2);
            total_descuento = matematica.aproxima(total_descuento +  producto.getImpDescuento(), 2);
            total_neto = matematica.aproxima(total_neto + producto.getImpNeto(), 2);
            total_iva = matematica.aproxima(total_iva + producto.getImpIva(), 2);
            total_subtotal = matematica.aproxima(total_subtotal + producto.getImpTotal(), 2);
        ...
        double diferencia_bruto = total_bruto - Double.parseDouble(encabezado.getTotales().getBruto());
        double diferencia_descuento = total_descuento - Double.parseDouble(encabezado.getTotales().getDescuento());
        double diferencia_neto = total_neto - Double.parseDouble(encabezado.getTotales().getNeto());
        double diferencia_iva = total_iva - Double.parseDouble(encabezado.getTotales().getIva());
        double diferencia_total = total_subtotal - Double.parseDouble(encabezado.getTotales().getTotal());
    ...
    private Encabezado buscarEncabezado(int pIdFactura, String pPantalla, String pOpcion, String pUsuario, String pTerminal){
        ...
        System.out.println("Buscar Total Bruto Factura: " + pIdFactura);
        
        float total_bruto_factura = totalBrutoFactura(pIdFactura);
        ...
        try {
            ...
            sql = "SELECT  c.nit_persona AS nit,\n"
                    ...
                    + "        '' AS numeroacceso,\n"
                    + "        'A' AS serieadmin,\n"
                    + "        a.id_factura AS numeroadmin,\n"
                    + "        '' AS reversion,\n"
                    + "        ROUND(" + total_bruto_factura + ", 2) AS bruto,\n"
    ...
    private Detalles buscarDetalle(int pIdFactura, String pPantalla, String pOpcion, String pUsuario, String pTerminal){
        ...
        try {
            ...
            sql = "SELECT  a.id_producto AS Producto,\n"
                    ...
                    + "        " + medida + " AS Medida,\n"
                    + "        ROUND(a.cant_d_factura, 2) AS Cantidad,\n"
                    + "        ROUND(b.precio_est_producto, 2) AS Precio,\n"
                    + "        ROUND((1 - (a.precio_venta_d_factura / b.precio_est_producto)) * 100, 2) AS PorcDesc,\n"
                    + "        ROUND((b.precio_est_producto * a.cant_d_factura ), 2) AS ImpBruto,\n"
                    + "        ROUND((b.precio_est_producto * a.cant_d_factura ) - (a.precio_venta_d_factura * a.cant_d_factura), 2) AS ImpDescuento,\n"
                    + "        ROUND(0, 4) AS ImpExento,\n"
                    + "        ROUND(0, 4) AS ImpOtros,\n"
                    + "        ROUND(((b.precio_est_producto * a.cant_d_factura ) - ((b.precio_est_producto * a.cant_d_factura ) - (a.precio_venta_d_factura * a.cant_d_factura))) / 1." + iva + ", 2) AS ImpNeto,\n"
                    + "        ROUND(0, 4) AS ImpIsr,\n"
                    + "        ROUND((a.precio_venta_d_factura * a.cant_d_factura), 2) - ROUND(((b.precio_est_producto * a.cant_d_factura ) - ((b.precio_est_producto * a.cant_d_factura ) - (a.precio_venta_d_factura * a.cant_d_factura))) / 1." + iva + ", 2) AS ImpIva,\n"
                    + "        ROUND((a.precio_venta_d_factura * a.cant_d_factura), 2) AS ImpTotal,\n"
                    + "        \"B\" AS TipoVentaDet,\n"
                    + "        \"\" AS DatosAdicionalesProd\n"
                    + "FROM    d_factura AS a,\n"
                    + "        m_producto AS b\n"
            ...
            sql = "SELECT  a.id_producto AS Producto,\n"
                    ...
                    + "        ROUND((a.precio_estandar_d_factura * a.cant_d_factura ), 2) AS ImpBruto,\n"
                    + "        ROUND((a.precio_estandar_d_factura * a.cant_d_factura ) - (a.precio_venta_d_factura * a.cant_d_factura), 2) AS ImpDescuento,\n"
                    + "        ROUND(0, 4) AS ImpExento,\n"
                    + "        ROUND(0, 4) AS ImpOtros,\n"
                    + "        ROUND(((b.precio_est_producto * a.cant_d_factura ) - ((b.precio_est_producto * a.cant_d_factura ) - (a.precio_venta_d_factura * a.cant_d_factura))) / 1." + iva + ", 2) AS ImpNeto,\n"
                    + "        ROUND(0, 4) AS ImpIsr,\n"
    ...
    private float totalBrutoFactura(int pIdFactura) {
        ...
        try {
            ...
            sql = " select ROUND(SUM(a.cant_d_factura * b.precio_est_producto),2) as total_bruto\n"
                    + "   from   d_factura as a,\n"
                    + "          m_producto as b\n"
                    + "   where  a.id_factura = " + pIdFactura + "\n"
                    + "      and a.id_producto = b.id_producto";

            System.out.println("Consultar total bruto: \n" + sql);

            AccesoInventario Acceso = new AccesoInventario();
            ResultSet rs = Acceso.listarRegistros(sql);

            while (rs.next()) {
                valor = rs.getFloat("total_bruto");
            }
            ...
            System.out.println("Ocurrio un error al buscar el total bruto de la Factura");
            System.out.println(Error.toString());

        }

        System.out.println("Se encontro un total bruto de " + valor);
